"""Internal compact projection used by the assessment pipeline.
...
class LegacyAssessmentProjection(TypedDict):
...
def assessment_problems(assessment: LegacyAssessmentProjection) -> list[str]:
