...
def reach_floor_decision(obs):
    ...
    an all-unpriced sheet collapses to 0.0 in ``coalesce(sum(usd_value), 0)``
    (B8.1a); an absent ``deployment_balance`` key now yields NO floor key at
    all (W3) -- handled by the caller's dedicated absent-key arm, not here.
    Both non-adopting arms yield ``value: None``.
    """
        ...
        for claim in claims:
            ...
            if cid == "flow.out":
                ...
                if rd and rv is not None:
                    ...
                    # proven bound", never a measured zero: an all-unpriced sheet
                    # collapses to 0.0 in ``coalesce(sum(usd_value), 0)`` (B8.1a),
                    # and an absent ``deployment_balance`` key now yields NO floor
                    # key at all (W3) -- the dedicated arm below handles it.
                ...
                elif str(obs.get("reach_indeterminate")).lower() == "true" and "observed_reach_floor_usd" not in obs:
                    # W3 three-state: the floor key's own ABSENCE is the third
                    # state -- no deployment_balance entry existed for the acting
                    # deployment, so there is no floor to state. Without this arm
                    # the chain falls through to cval, whose .get() default would
                    # mint a magnitude out of an honest absence.
                    inst_val = None
                    val_basis = "observed_reach_floor_absent(not_determined)"
                    warnings.append(
                        {
                            "kind": "reach_floor_absent",
                            **ctx,
                            "capability": cid,
                            "note": (
                                "reach_indeterminate with no observed_reach_floor_usd "
                                "key: no balance row existed for the acting deployment "
                                "(W3 three-state); no proven bound, and no default "
                                "magnitude may stand in for one"
                            ),
                        }
                    )
