...
| `flows[].target_variable` / `target_variables` | 6 producer rows on the corpus; the 2 `storage_setter` gap rows are 423, 2748 | CONF (cite) | the destination is no longer nameless (B20.2). Scalar only when the kind NAMES a state variable AND every site named the same **declaration** — compared on canonical `Contract.var`, never on the bare string, or two contracts each declaring `recipient` publish as one. On disagreement the scalar is ABSENT and `target_variables` lists the **canonical** members — take the worst, never one as the whole. A mapping/array **element** emits neither |
| `storage_determined` | still LIVE as a `static_destination_shape` value: produced at `calldata.py:1119`, defined `services/effects/config.py:59`, read `recipes.py:1527`, emitted as `destination_storage_determined_deferred` by `scorer_v3.py:621/1525/1531`; **0 rows in the persisted `claims` jsonb**, where the value is `target_kind.kind = 'storage_setter'` (population 2) | defer | the deferral's CAUSE is now addressed — `target_variable` + `target_writer_signatures` name the destination and its in-unit writers, so the setter's principal is resolvable through `function_principals`. The shape token itself is unchanged; do not read this row as removed |
| `flows[].target_writer_signatures` (+ `target_writer_scan_complete`) | 2 | CONF (cite) — **a FLOOR** | "redirectable **at least by** these writers' principals, within the analysed compilation unit" — never the closed set. `[]` ONLY under `storage_no_setter` (the kind is the completed-scan negative); **key absent = no callable writer attributed** — and `target_writer_absent_reason` (next row) says which of the two causes it was. The gate travels in the same dict: `scan_complete=false` means an assembly `sstore`/`delegatecall`/unresolved alias left the attribution non-exhaustive |
| `flows[].target_writer_absent_reason` | 0 | CONF (diagnostic, **no consumption obligation**) | `declaration_initialiser_only` (the only writer is Slither's synthetic constructor initialiser, which is not invocable — kept in setter MEMBERSHIP so the row does not flip to `storage_no_setter` and mint a false earned negative, but never published as a principal) \| `alias_unattributed` (a storage alias contributed the name with no attributable function). **Mutually exclusive with the payload** — it is emitted on the arm that returns before `target_writer_signatures` and `target_writer_scan_complete`, so no completed-scan gate can ride beside it. `writer_surface_closed: not_determined` and `target_kind: storage_setter` still ride on the row, so the write surface stays unbounded. **Neither value may move a weight.** Full entry: `SCORING_INVARIANTS.md` B20.2 |
...
| `witness.sink_ids` | **107** entries on `flow.out` claims (**217** across all protocol-1 claims — the scope `SCORING_INVARIANTS.md` B20 counts); 0 addresses in either | BAN for asset identity | carries a variable name only; name-based resolution is inv.2-banned. **Read `witness.sink_receivers` instead** — same sinks, keyed by the same ids. **The sink→flow join is per identity regime (2026-07-30, fix 7):** an `out`/`in` flow joins on its own selector; a `value_router` flow joins on its `router_ops` identity, because its selector names the CALLEE's inner transfer — so a same-selector DIRECT call in the entry can no longer lend a routed claim its receiver, and a routed flow with no recorded op matches nothing (keys absent = `not_determined`) |
| `witness.sink_receivers{sink_id}.{binding,param_scope,param_index,mutability,visibility,auto_getter_selector,variable,receiver_provenance,not_determined_reason}` | 57 of 76 external-call sinks on the corpus (the 19 without are low-level `.call`, where the key is ABSENT = never computed); the 18 gap rows split 4 entry-parameter / 3 helper-formal / 10 state-variable / 1 local | GATE (+CONF members) | full semantics in `SCORING_INVARIANTS.md` **B20.1**. The three that matter here: `binding` is decided by `isinstance`, **never** by `visibility` (a `LocalVariable` and an internal state var are indistinguishable on it); `auto_getter_selector` is licensed by the **declared type** and only for a NULLARY public getter (`uint256[] public amounts` → `amounts(uint256)`, not `amounts()`); and **`variable` is display/identity only — `variable + "()"` is the banned shape.** No address is published on this plane |
| — `token_identity` precondition | — | REQ | now **decidable**: satisfied only by `receiver_provenance = contract_state_unresolved` + a resolved address + a non-`not_determined` invariant. **FAILS** for `caller_named` (4 rows lose single-asset pricing — a demotion, in the honest direction), for `not_determined`, and for an absent `receiver` |

## 4. Principals and protection

| field | pop. | status | notes |
|---|--:|---|---|
| `function_principals.details->>'delay'` | **139/139**, 0 nulls | REQ | three timelocks: 864000 / 432000 / 172800. **Honest strength is duck-type-plus-negative-control**: a nullary uint256 getter named `getMinDelay` **or `delay`** answered, and the address is not a catch-all. Which selector answered is **not recorded**, and one timelock has no structural corroboration at all. **SCOPED 2026-07-30 (C2 / Unit 9): the negative control is a PRODUCER INVARIANT on rows written after 382d81dc (#161)** — both `details["delay"]` writers (`tracking.py:913` batched, `:1013` unbatched) sit behind `_duck_type_permitted()`, but the observed run wrote 19:57:30Z–21:44:17Z on 2026-07-29 and `_NEGATIVE_CONTROL_SIG` first appears at 2026-07-30 00:28:56Z. **The 53 rows carrying `delay=172800` today have no control witness and no probe_block for that read** (their `trace[0].probe_block` belongs to a role fold on a different address, and is 12 distinct values spanning 25641034–25641184, not the single 25641184 an earlier draft quoted), so that delay may not be cited as a proven ordinal until the C2 job rewrites them. See `SCORING_INVARIANTS.md` B10 item 6b |
...
| `details->'safe_protection'` (`probe_block`, `safe_version`, `modules_head`, `module_set`, `module_set_basis`, `protection_is_upper_bound`, `guard`) | 19 applicable | REQ, three-state | the module/guard witness at a pinned height. `module_set == []` is **proven empty** (head == sentinel, basis `storage_linked_list_terminated`) and licenses k/n **at `probe_block` only**; anything else — non-sentinel head, zero word, unreadable **or malformed** word (the storage words are decoded strictly: exactly 64 nibbles, `bytes.fromhex`, never left-padded first — `"0x"` padded is the zero word and `"0x1"` padded IS the sentinel), unresolvable height — is `not_determined`, and **an enumerated non-empty array is never published from the head word**. `protection_is_upper_bound` is `true` (proven module present ⇒ demote the k/n credit) or `not_determined`, **never `false`**: "no module ever enabled" needs a warm `EnabledModule` cursor from creation. `guard ∈ proven_address / proven_zero (1.3.0, 1.4.1 only) / feature_absent (1.1.1 — the slot is unused storage there) / not_determined`. **B14: 1 module-bearing Safe, reach 4 rows, `protocol_id IS NULL` — cite, never calibrate.** Full entry: `SCORING_INVARIANTS.md` B10.1a |
| `control_graph_nodes.details->'gated_contract_backlink'` (`probe_block`, `backlink_getter`, `gated_contract_address`, `backlink_address`, `negative_control`, `declared_vault_matches_gated_contract`) | **38 projections** (32 addrs / 37 marked CGN rows / 32 M / 17 V) — **20 `true` / 18 not_determined** at 25643300 (17 nd counted over marked rows; the `details->>'source'` lens undercounts firings by one, see B10.1b) | REQ, three-state | **NEW 2026-07-30 (C3 / Unit 9).** The role-principal PAIRING witness at a pinned height, giving the (M, V) pairing a structural basis instead of the `ManagerWithMerkleVerification` **label** (inv.2 / T-LABELS). **It does NOT introduce the `vault()` read** — the pipeline already reads it via the `external_contract` controller path (`tracking_plan.py:86`), persisted as 32 `control_graph_edges` `controller_value`/`label='vault'` rows over 25 contracts with `controller_values.block_number`; a first draft of this entry wrongly said "probed nowhere", and lane C was substantially right. What is new is the **verdict**: equality against the specific V, behind a negative control, as an auditable three-state. `declared_vault_matches_gated_contract` is `true` or `not_determined` with **no false state**; the **mismatch payload is byte-identical to the never-read payload**, so this object carries no mismatch signal at all. **Three bans:** (1) it does NOT establish M's type — **10 of the 20 `true` rows are NOT managers** (Tellers/solvers/vaults: `0x35dd2463fa7a…`, `0x417e1ef6…`, `0x4de413a2…`, `0x6ee3aacc…`, `0x8ea0b382…`, `0x929b44db…`, `0x99de9e5a…`, `0x9aa79c84…`, `0xd445c65e…`, `0xe238e253…`), so reading it as a type mis-types half the positive population; (2) a mismatch is not an earned negative — a **prose ban on the consumer**, not a guarantee of unknowability, since the raw address is published on the `controller_value` plane and a join can reconstruct it; (3) "the manager is analysed" must never become "the manage capability is bounded" (`manageRoot` is a root hash). Cost ~2 RPC per pair, **×2** because `resolve_control_graph` runs in both resolution and the policy refresh. **B14: 20 `true` rows, 17 distinct V — cite/gate, never calibrate.** Full entry: `SCORING_INVARIANTS.md` B10.1b |
| `selection_summary.not_selected` / `.pre_rank_excluded` / `perimeter_spawn_summary` | per job | REQ (cite) | **NEW 2026-07-30 (C2+C3 / Unit 9).** The omission ledgers. Nothing was dropped **iff `not_selected == [] AND pre_rank_excluded == []`** — `not_selected` alone does not support the claim, because two filters (the `effective_confidence >= 0.3` threshold and the superseded-impl anchor predicate) run upstream of the ranking and previously left only a count / an invisible SQL exclusion. **Population: `Contract.protocol_id == job.protocol_id AND job_id IS NULL` (`selection_worker.py:182-185`) — a `protocol_id IS NULL` orphan is invisible to BOTH ledgers and lies outside the claim.** The spawn summary's three dispositions (`queued` / `omitted` / `out_of_population`) **partition** the node list **only when `walked == true`**; on `walked == false` (or a ledger with no `walked` key) they are a **prefix** and their emptiness proves nothing — read coverage as `not_determined`. `out_of_population` is the fail-closed + dedup set and is **not** an omission. `perimeter_spawn_summary` is written on **every** policy job (an absent artifact means "predates the ledger", never "omitted nothing") and from a `finally`, so a `create_job` raise part-way through still records the children already committed — and, since that `finally` also fires when the walk NEVER RAN (`resolved_control_graph` is not a dict) or raised mid-walk, `walked` (set at loop exit only) is what distinguishes those from a completed walk that omitted nothing. Budget decrements at `create_job` only, and in the selection walker the budget is checked **after** the chain/dedup gates so a below-the-cut candidate reports its real cause rather than `budget_exhausted`. `PERIMETER_SPAWN_LIMIT=8` / `PERIMETER_SPAWN_DEPTH_CAP=2` are **named model choices, never calibrated** (B14; population 19). `spawn_depth` counts spawn GENERATIONS and is **not** `control_graph_nodes.depth` (=1 on all 19). Full entry: `SCORING_INVARIANTS.md` B10.1c |
...
6. **2-day timelock** (`contracts.id=11`) — 0 `effective_functions` while gating 53 rows across 16 contracts. **Diagnosed and instrumented 2026-07-30 (C2 / Unit 9); the row itself still needs the job.** The cause was not a bug: it ranked 0.3836 at queue positions 42-57 against `analyze_limit=2` and the budget cut it **silently**. The fix is ONE job plus a ledger (`selection_summary.not_selected` / `.pre_rank_excluded`, §4), explicitly **not** a raised `analyze_limit`. The expected output is pinned in advance and verified offline against the real verified source at solc 0.8.25: exactly the 12 non-view names both twins produced (`tests/test_timelock_surface_parity.py`). **`authority_openness` is deliberately NOT pinned** — the twins' 8 `restricted` verdicts are `finite_set` capabilities carrying deployment-specific member addresses from on-chain role folds, so asserting them for id=11 would import the twins' role holders onto a different contract. Until the job runs, publication stays `proposer_set: not_determined` / `timelock_proposer_unresolved`. **Live-run request:** `{"address": "0xcd425f44758a08baab3c4908f3e3de5776e45d7a", "chain": "ethereum", "protocol_id": 1, "name": "Operating Timelock"}` (`find_existing_job_for_address` returns None — 0 jobs match, and `jobs.address` carries no uppercase anywhere, so this is not a casing miss).
8. ~~**Policy-refresh perimeter**~~ — **LANDED 2026-07-30 (C3 / Unit 9).** The policy stage rewrote the refreshed control graph but never called the spawn, so every node FIRST discovered there — every `role_principal`, since role principals need the `effective_permissions` that stage computes — was permanently outside the analysis perimeter. Measured: 32 `semantic_capability:role_grant` contract addresses, **19 jobless**, all `analyzed=true`, i.e. all 19 already satisfied every gate the spawn applies. One shared walker now serves both call sites (`services/discovery/perimeter.py`), budgeted and depth-capped because the policy path is recursive, with every cut recorded. Only 10 of the 19 carry the manager label — the spawn keys on the persisted provenance field, never the label.
...
| `executor_kind` | 3-valued, never NULL | REQ (gate + cite) | `timelock_routed` / `safe_direct` / `not_determined`. A positive needs three things at once: a keccak-matched marker log, an emitter the **persisted classification plane** independently typed **on the receipt's own chain** (every plane read joins `contract_id → Contract.chain`; an unresolvable chain is dropped, never assumed local), and a provably complete log set. Unclassified emitter ⇒ `not_determined` (the 11 measured transactions on the unmodelled Safe `0xf155a263…` are exactly this) |
...
| `executor_classification_block` | **0 today** | REQ | `not_determined` on every current row; from B10.1a's `safe_protection.probe_block` going forward, **taken only from SAME-CHAIN rows** — another chain's probe height is no citation here and the field stays absent. Why it matters: the kind asserts the emitter is typed a Safe/timelock **by our classifier**, not that it was one at the upgrade's block |
...
| field | pop. | status | notes |
|---|--:|---|---|
| `holders` | 11 keys / 3 registries | REQ | a JSON array of **plain lowercase `0x` address strings** — no per-element block, because `as_of_block` is a row column and every member shares it. A **floor**. `len(holders)` is never a count. NULL = `not_determined`; **`[]` is unrepresentable** (DB CHECK). Revert, transport failure, cold cursor, unpinnable block, or zero confirmations all ⇒ NULL |
...
| `as_of_block` | 11 | REQ | pinned at `head − DEFAULT_CONFIRMATION_DEPTH` (12) and asked for by number. Never `"latest"`; an unpinnable height withholds the probe entirely |
| `as_of_block_hash` | best-effort | CONF | the pinned block's hash **when readable AND a full 32 bytes; NULL otherwise**. A failed hash read, `"0x"`, a short/over-long word or non-hex all keep the height and leave the hash NULL — only replay-after-reorg is weaker. **Never `b""`** |
...
| `role_name` | **10/11** (8 keccak + 2 literal; only `0x5f58e3a2` TIMELOCK_ADMIN_ROLE unnamed) | **CONF** | proves *a preimage of `role_hash` is the string S* — a total mathematical fact — **not** that this registry declares a constant named S. Cite only; never gates, never keys. NULL = key absent |
...
| `fold_chain_disagreements` | 11 | CONF | keys exactly `{registry, role_hash, address, fold_state, chain_state}`. Recorded, **never diagnosed** — `as_of_block` is above the cursor head, so cause is `not_determined`. Array = observed among the candidates whose reads COMPLETED — and a read **completes only on a full 32-byte word**, so a success carrying `"0x"` (what `eth_call_batch` reports for an unreadable node result) is `unconfirmed`, never `chain_state: "false"`; **NULL on a withheld row, never `[]`** (nothing was read, or what was read is withheld with the floor it qualifies) |
...
| minting rows from `spec_by_topic0()` / `STANDARDS` | it also carries Solady `RoleSet` (`role_topic_index=2` vs OZ's 1). Measured: 125 logs / 40 role words / ONE `RoleSet` cursor on that protocol-1 registry (the corpus's 5 `RoleSet` cursors sit on 5 different addresses) ⇒ the plane would go 11 → 51 keys, and each Solady key's `hasRole` **succeeds** returning the mapping's **zero default** — a completed read witnessing nothing. Topic gate is LITERAL |
...
**Not wired — populations are MEASURED, NOT PERSISTED.** Nothing calls
`resolve_role_holder_planes` / `persist_role_holder_planes` outside tests, and
`role_holder_planes` is empty in every environment. Every count above was
measured by running the resolver in-process against the local replica plus
pinned chain reads. A **deferral with cause**: plan §1.1 forbids running the
orchestrator, so a worker wiring could not be validated end-to-end and an
unexercised writer was not put on a production path. **An absent row is a
statement about wiring, not about roles.**

**`role_name` can be GAINED later** when an unrelated contract declaring a
matching constant is analysed — an addition, not a correction, because absence
is `not_determined` throughout and a proven preimage never stops being true. Do
not cache its absence.
