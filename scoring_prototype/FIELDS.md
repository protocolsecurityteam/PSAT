...
| `role_definitions.role_name` | 19 rows / 11 protocol-1 | CONF | **CORRECTED 2026-07-30 (D6-reject): ids 1 and 19 were ERC-7201 storage-layout pointers, not roles, and are no longer written.** A `bytes32` constant is admitted only where it is witnessed as the **KEY** of the set the leaf tests, on two arms, both requiring an empty `member_path`: (1) **mapping** — `kind=="membership"` + `mapping_membership` descriptor; (2) **external** — `kind=="external_bool"` + `external_set` descriptor whose `callee_selector` is exactly `0x91d14854` (`hasRole(bytes32,address)`) **and** whose `key_sources` put the constant in the role argument (0) and a caller-tainted operand in the account argument (1). **The selector+position gate is load-bearing:** `key_sources` is built from every call argument, so admitting on the descriptor kind alone lets a slot constant (`readBool(address,bytes32)`), a merkle root (`isInTree(bytes32,address)`) and a CREATE2 salt (`isAuthorized(address,bytes32)`) all mint as roles — measured through the real pipeline. Row absence is `not_determined`, never "no roles": 50 role-keyed gates carry the role as a `view_call`/`parameter` operand and never reach this table. **Banned:** the `_is_storage_layout_constant` name-suffix guard (wrong both ways) and gating on `storage_var=='_roles'`/`enumeration_hint` (drops 5 real Lido roles). Full entry: `SCORING_INVARIANTS.md` B4c |
