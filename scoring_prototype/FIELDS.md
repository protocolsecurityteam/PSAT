...
| `control_graph_nodes.details->'gated_contract_backlink'` (`probe_block`, `backlink_getter`, `backlink_address`, `negative_control`, `declared_vault_matches_gated_contract`) | 32 applicable / 10 expected `true` | REQ, three-state | **NEW 2026-07-30 (C3 / Unit 9).** The role-principal PAIRING witness, at a pinned height, replacing the `ManagerWithMerkleVerification` **label** (inv.2 / T-LABELS) as the basis for pairing a role-granted contract M with the contract V whose function it gates. Earns exactly: *M declares V as its `vault()` at `probe_block`, with the nonsense-selector control passed* — reproduced **10/10** at block 25643300, control passing **10/10**. `declared_vault_matches_gated_contract` is `true` or `not_determined` and **has no false state**; every revert / empty / non-32-byte / decode-failure / transport-error / failed-or-errored control / **mismatch** lands on `not_determined`, and an unpinnable height makes the **whole key absent**. **Three bans:** (1) it does NOT establish M's component type nor identify M as "the manager" — measured counterexample `0x35dd2463…`, a Teller, back-links to the same V with the control passing and would publish `true`; (2) a mismatch is NOT an earned negative — enforced structurally, since `backlink_address` is published only on a match so the mismatch cannot be reconstructed; (3) "the manager is analysed" must never become "the manage capability is bounded" — `manageRoot(strategist)` is a root hash, so the strategist's admissible call set stays `not_determined`. **B14: 10 confirming rows, 2 bytecode variants — cite/gate, never calibrate.** Full entry: `SCORING_INVARIANTS.md` B10.1b |
| `selection_summary.not_selected` / `.pre_rank_excluded` / `perimeter_spawn_summary` | per job | REQ (cite) | **NEW 2026-07-30 (C2+C3 / Unit 9).** The omission ledgers. Nothing was dropped **iff `not_selected == [] AND pre_rank_excluded == []`** — `not_selected` alone does not support the claim, because two filters (the `effective_confidence >= 0.3` threshold and the superseded-impl anchor predicate) run upstream of the ranking and previously left only a count / an invisible SQL exclusion. The spawn summary's three dispositions (`queued` / `omitted` / `out_of_population`) **partition** the node list; `out_of_population` is the fail-closed + dedup set and is **not** an omission. Budget decrements at `create_job` only, so an earlier gate provably consumes none. `PERIMETER_SPAWN_LIMIT=8` / `PERIMETER_SPAWN_DEPTH_CAP=2` are **named model choices, never calibrated** (B14; population 19). `spawn_depth` counts spawn GENERATIONS and is **not** `control_graph_nodes.depth` (=1 on all 19). Full entry: `SCORING_INVARIANTS.md` B10.1c |
