...
| `witness.sink_ids` | **107** entries on `flow.out` claims (**217** across all protocol-1 claims — the scope `SCORING_INVARIANTS.md` B20 counts); 0 addresses in either | BAN for asset identity | carries a variable name only; name-based resolution is inv.2-banned. **Read `witness.sink_receivers` instead** — same sinks, keyed by the same ids |
...
| `as_of_block_hash` | best-effort | CONF | the pinned block's hash **when readable; NULL otherwise**. A failed hash read keeps the height — only replay-after-reorg is weaker |
