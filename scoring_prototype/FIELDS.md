...
| `flows[].router_ops[].{selector,callee}` | 0/41 applicable `value_router` claims in this snapshot — projected as of U5/D5, stored rows gain it on re-analysis; producer 73/73 routed flows | CONF (cite) | the identity of the call(s) carrying a routed move — the routed flow's own `selector` is the CALLEE's inner transfer, so this is the only identity of the call this function makes. `selector` = keccak4 of the callee's canonical signature; **`callee` is an intra-unit AST name, NEVER a resolved on-chain target** (an interface-typed parameter hashes differently, which is why the name travels with it) — join it to this unit's functions and nothing else. **Key absent = `not_determined`** and is the only failure state (unrecorded, pre-field artifact, or unrouted); `[]` is never emitted, because an empty op list would read as "no router" and license the transparency absence denies. A routed flow with the key absent stays fail-closed: its router leaf blocks and the mandatory gate falls to `not_determined`. **No proven-absent state exists** — do not infer "crosses no call" from absence |
...
| `details->>'threshold'` + `details->'owners'` | 454/454 | REQ | live `getOwners()`/`getThreshold()`. k/n is **exact** on this corpus (modules and guard probed empty on all 9 Safes) — but that is a one-block observation needing an `EnabledModule`/`ChangedGuard` monitor |
...
`role_definitions.role_name` | mis-parses ERC-7201 slot constants as roles. The 9 real names are **not** in the authority plane by name or keccak — log as a coverage gap; the grants are witnessed in `indexed_event_logs` and unused
...
7. **Safe modules and guard** — two more probes in the existing batched classifier (`getModulesPaginated`, the guard storage slot). Currently k/n is an upper bound in principle.
