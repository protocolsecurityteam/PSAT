...
VALUE_CLAIMS = {
    "flow.out", "flow.in", "erc20.transfer", "erc20.transfer_from",
    "supply.mint", "supply.burn",
}
...
def freeze_severity(claim, unset_exists, unset_princs, pauser_addr, warn, ctx):
    ...
    if not unset_princs:
        warn.append({
            "kind": "freeze_recovery_principal_unresolved", **ctx,
            "note": ("pause.unset exists but no unpause principal resolved; "
    ...
    warn.append({
        "kind": "freeze_recovery_not_independent", **ctx,
        "note": ("the only resolved unpauser is the pauser itself; this key can "
                 "sustain its own freeze"),
...
def main():
    ...
    for f in funcs:
        ...
        for claim in claims:
            ...
            if cid == "flow.out":
                ...
                if dshape == "caller_arbitrary":
                    ...
                    if rv is not None:
                        ...
                        inst_val = 0.0
                        warnings.append({
                            "kind": "extraction_magnitude_unwitnessed", **ctx,
                            "capability": cid,
                            "note": ("destination proven caller_arbitrary but "
                    ...
                    warnings.append({
                        ...
                        "capability": cid,
                        "note": ("flow.out with destination_shape "
                ...
                sev, sev_reason = freeze_severity(
                    claim, f["cid"] in unset_exists,
                    unset_by_c.get(f["cid"], set()), None, [], ctx)
                ...
                if bn and vfns:
                    ...
                    warnings.append({
                        "kind": "freeze_blast_radius_unwitnessed", **ctx,
                        "capability": cid,
                        "note": ("pause capability with no observed_blast_radius; "
                ...
                freeze_facts.append({
                    **ctx, "severity": sev, "reason": sev_reason,
            ...
            for p in fprinc:
                    ...
                    warnings.append({
                        ...
                        **ctx, "capability": cid, "principal": p["addr"],
                        "note": (f"{cid} gated by a {lab} principal whose own "
                                 "authority is not reduced to a key"),
                ...
                if cid == "pause.set":
                    ...
                    p_sev, p_reason = freeze_severity(
                        claim, f["cid"] in unset_exists,
                        unset_by_c.get(f["cid"], set()), p["addr"], warnings, ctx)
            ...
            if not scored_any and not fprinc:
                warnings.append({
                    "kind": "restricted_no_principal", **ctx, "capability": cid,
    ...
    for f in findings:
        ...
        if f["capability"] in TRANSITIVE_CAPS:
            ...
            for a in f["reach_addrs"]:
                addrs[a] = min(val_by_addr.get(a, 0.0), f["value_at_stake_usd"]) \
                    if f["value_at_stake_usd"] else val_by_addr.get(a, 0.0)
    ...
    if rec_ok:
        ...
        overlap = [x for x in rec_ok if x["blast_radius"]
                   and any((x["address"], fn) in scored_fns for fn in x["blast_radius"])]
        credits.append({
            "protective_fact": ("emergency brake: pause capability with a witnessed, "
            ...
            "counterfactual_if_removed": ("deleting the brake removes these rows "
                                          "entirely; proto-0.1 paid +7.77 grade points "
                                          "for having no pause at all"),
            "mitigation_overlap": [
                {"contract": x["contract"], "pause_function": x["function"],
    ...
    headline = None
    if top:
        headline = (f'{top["weakest_gate"]} can {top["capability"]} on '
    ...
    out = {
        ...
        "grade_exposure": grade_exposure, "grade_exposure_letter": letter(grade_exposure),
        "confidence_pct": confidence_pct,
        "total_tracked_value_usd": round(total_value),
        "total_exposure_usd": round(exposure),
        "decomposition_check": {
            "lambda": {"grade": grade_lambda,
                       "sum_net": round(sum(f["net_points_lambda"] for f in findings), 3),
                       "residual": round(100.0 - grade_lambda
                                         - sum(f["net_points_lambda"] for f in findings), 6)},
            "exposure": {"grade": grade_exposure,
    ...
    for f in findings[:14]:
        print(f'{f["raw_points"]:7.2f} {f["net_points_lambda"]:7.2f} '
...
def confidence(funcs, princ, contracts, val_by_addr):
    ...
    for f in funcs:
        ...
        den += b
        if (claims and (f["openness"] == "open" or princ.get(f["fid"]))
                and f["openness"] != "not_determined"):
            num += b
    return num, den
