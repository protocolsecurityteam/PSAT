#!/bin/bash
# Per-unit isolated offline test runner (worktree u10a).
# Usage: ./run_tests.sh [pytest args, e.g. tests/test_x.py]
# Do NOT source .env around this; it exports its own CI-faithful env.
set -e
cd "$(dirname "$0")"
N="${PSAT_XDIST_N:-2}"
export TEST_DATABASE_URL="postgresql://psat:psat@localhost:5433/psat_test_u10a"
export DATABASE_URL="$TEST_DATABASE_URL"
export TEST_ARTIFACT_STORAGE_ENDPOINT="http://localhost:9000"
export TEST_ARTIFACT_STORAGE_BUCKET="psat-artifacts-test-u10a"
...
if ! PGPASSWORD=psat psql -h localhost -p 5433 -U psat -d postgres -tAc       "SELECT 1 FROM pg_database WHERE datname='psat_test_u10a'" 2>/dev/null | grep -q 1; then
  PGPASSWORD=psat psql -h localhost -p 5433 -U psat -d postgres -c "CREATE DATABASE psat_test_u10a OWNER psat;" >/dev/null
fi
