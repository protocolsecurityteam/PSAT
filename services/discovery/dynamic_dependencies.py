...
def _require_trace_address(address: Any, *, field: str, tx_hash: str) -> str:
    normalized = _normalize_maybe_address(address)
    if normalized is None:
        raise ValueError(f"trace {tx_hash} has malformed {field} address: {address!r}")
    return normalized
...
def fetch_contract_transactions(address: str, *, chain_id: int, limit: int = 30, start_block: int = 0) -> list[dict]:
    """Fetch recent normal and internal transactions for an address.
...
def resolve_trace_rpc(*, chain_id: int) -> str:
    """Resolve a tracing-capable eRPC URL for a chain id."""
    load_dotenv(Path(__file__).resolve().parent.parent / ".env")
    return default_rpc_url(chain_id=chain_id)


def _fetch_tx_metadata_from_rpc(rpc_url: str, tx_hash: str, *, chain_id: int) -> dict:
    tx = rpc_call(rpc_url, "eth_getTransactionByHash", [tx_hash], retries=1, chain_id=chain_id)
...
def trace_transaction(rpc_url: str, tx_hash: str, *, chain_id: int) -> tuple[str, Any]:
    """Trace a transaction through the canonical eRPC debug trace method."""
    method = "debug_traceTransaction"
    params = [tx_hash, {"tracer": "callTracer", "timeout": "20s"}]
    try:
        return method, rpc_call(rpc_url, method, params, retries=0, chain_id=chain_id)
    except RuntimeError as exc:
        logger.error("debug_traceTransaction failed for tx=%s: %s", tx_hash, exc)
        raise RuntimeError(f"debug_traceTransaction failed for {tx_hash}") from exc


def _parse_debug_call_tree(node: Any, tx_hash: str, block_number: int | None, out_edges: list[dict]) -> None:
    if not isinstance(node, dict):
        raise ValueError(f"debug trace {tx_hash} has malformed call node: {node!r}")

    op = str(node.get("type", "")).upper()
    if op in TRACE_OPS:
        src = _require_trace_address(node.get("from"), field="from", tx_hash=tx_hash)
        dst = _require_trace_address(node.get("to"), field="to", tx_hash=tx_hash)
        out_edges.append(
            {
                "from": src,
                "to": dst,
    ...
    for child in node.get("calls", []) or []:
        if not isinstance(child, dict):
            raise ValueError(f"debug trace {tx_hash} has malformed child call node: {child!r}")
...
def _parse_parity_trace_entries(entries: Any, tx_hash: str, block_number: int | None, out_edges: list[dict]) -> None:
    if not isinstance(entries, list):
        raise ValueError(f"parity trace {tx_hash} has malformed trace list: {entries!r}")

    for item in entries:
        if not isinstance(item, dict):
            raise ValueError(f"parity trace {tx_hash} has malformed trace item: {item!r}")
        trace_type = str(item.get("type", "")).lower()
        action = item.get("action", {}) or {}
        if not isinstance(action, dict):
            raise ValueError(f"parity trace {tx_hash} has malformed action: {action!r}")
        ...
        if trace_type == "call":
            ...
            src = _require_trace_address(action.get("from"), field="action.from", tx_hash=tx_hash)
            dst = _require_trace_address(action.get("to"), field="action.to", tx_hash=tx_hash)
            ...
            if not isinstance(result, dict):
                raise ValueError(f"parity trace {tx_hash} has malformed create result: {result!r}")
            src = _require_trace_address(action.get("from"), field="action.from", tx_hash=tx_hash)
            dst = _require_trace_address(result.get("address"), field="result.address", tx_hash=tx_hash)

        if op in TRACE_OPS and src is not None and dst is not None:
            out_edges.append(
                {
                    "from": src,
                    "to": dst,
...
def find_dynamic_dependencies(
    ...
    *address* (the implementation).
    ...
    trace_rpc = resolve_trace_rpc(chain_id=chain_id)

    if tx_hashes:
        selected_txs = [
            _fetch_tx_metadata_from_rpc(trace_rpc, tx_hash.strip(), chain_id=chain_id) for tx_hash in tx_hashes
        ]
    else:
    ...
    parallel_traces = parallel_map(
        lambda tx: trace_transaction(trace_rpc, tx["tx_hash"], chain_id=chain_id),
        selected_txs,
        max_workers=10,
    )
    for tx, result in parallel_traces:
        ...
        if isinstance(result, RuntimeError):
            ...
            logger.error("Trace failed for tx=%s chain_id=%s: %s", tx_hash, chain_id, result)
            raise RuntimeError(f"trace failed for tx={tx_hash} chain_id={chain_id}") from result
        ...
        try:
            ...
            logger.error(
                "Trace parsing failed for tx=%s chain_id=%s method=%s: %s",
                tx_hash,
                chain_id,
                method,
                exc,
            ...
            raise RuntimeError(f"trace parsing failed for tx={tx_hash} chain_id={chain_id}") from exc
    ...
    def _cached_get_code(addr: str) -> str:
        if addr not in code_cache:
            code_cache[addr] = get_code(trace_rpc, addr, chain_id=chain_id)
