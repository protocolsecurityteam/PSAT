...
def _scalar_owner_vars(contract: Any, predicate_trees_artifact: Any) -> set[str]:
    """The owner-var harvest, restricted to hygiene-clean scalar address
    pointers. A ``caller_authority`` equality can resolve to the bytes32
    *slot constant* of an OZ v5 namespaced-storage owner (``OwnableStorageLocation``),
    which Slither then reports as "written" by every touching function
    (including the ``owner()`` view). Dropping constants / bytes32 / mapping /
    array operands keeps the real ``address _owner`` pointers and drops the
    ghosts."""
    raw = _owner_vars_from_predicate_trees(predicate_trees_artifact)
    if not raw:
        return set()
    by_name = {getattr(variable, "name", ""): variable for variable in _all_state_variables(contract)}
    clean: set[str] = set()
    for name in raw:
        ...
        if bool(getattr(variable, "is_constant", False)):
            continue
        type_str = str(getattr(variable, "type", "") or "")
        if type_str.startswith("mapping") or "[]" in type_str or type_str == "bytes32":
            continue
        clean.add(name)
    return clean
...
def apply_authority_effect_labels(
    ...
    pointer). The harvest is gated on hygiene-clean scalar address vars
    (``_scalar_owner_vars``), and view/pure functions are skipped — an OZ v5
    ``owner()`` getter that Slither reports as "writing" the slot constant is
    not an ownership transfer. Supersedes the coarse ``hook_update`` /
    ``external_contract_call`` fallbacks on the tagged function and refreshes
    its action summary. Mutates ``effects_artifact`` in place; a no-op if
    either artifact errored."""
    functions = effects_artifact.get("functions") if isinstance(effects_artifact, dict) else None
    if not isinstance(functions, dict):
        return
    owner_vars = _scalar_owner_vars(contract, predicate_trees_artifact)
    if not owner_vars:
        return
    for fn in getattr(contract, "functions", []) or []:
        if _is_view_or_pure(fn):
            continue
