...
from .constants import STANDARD_EVENTS, STANDARD_SIGNATURES
from .shared import (
    _all_modifiers,
...
def _effect_records_with_claim(effects: Mapping[str, Any] | None, claim_id: str) -> list[tuple[str, dict[str, Any]]]:
    ...
    for signature, info in (effects.get("functions") or {}).items():
        if not isinstance(signature, str) or not isinstance(info, dict):
            continue
        if any(claim.get("claim_id") == claim_id for claim in info.get("claims") or [] if isinstance(claim, dict)):
...
def _build_semantic_control_summary(
    ...
    for function in functions:
        ...
        # Source effect records from the per-function effects artifact. If it
        # is missing, leave the summary empty rather than inferring a second path.
...
def _detect_upgradeability(
    ...
    update_records = _effect_records_with_claim(effects, "upgrade.implementation")
    functions_by_signature = {
