...
describe("DetailEmptyState — score card", () => {
  it("renders the calibrated grade, ledger and fix-first from the published document", async () => {
    ...
    expect(screen.getByText(/54\.8 \/ 100/)).toBeInTheDocument();
    expect(screen.getByText(/confidence 20\.7%/)).toBeInTheDocument();
    expect(screen.getByText(/54\.8 kept/)).toBeInTheDocument();
    expect(screen.getByText(/across 19 findings/)).toBeInTheDocument();
    // The fix-first sentence is the derivation the score page publishes.
    expect(screen.getByText(/Move the two EOA authority holes/)).toBeInTheDocument();
