...
# A genuine view with three external-call heads. It carries NO derived state write, so nothing about it is
...
def test_negative_control_view_gains_no_state_write_evidence() -> None:
    """A view with external-call sinks must not invent a state write."""
...
def test_the_witness_survives_the_permission_index_artifact() -> None:
...
def test_artifact_with_no_effects_input_publishes_not_determined_everywhere() -> None:
    """``build_permission_index(effects=None)`` is what the worker calls
