...
rule — is this node's sheet determined, and is its key one two proxies share —
and the corpus cannot exercise it. Protocol 1 carries no proven-empty sheet and
no ambiguous alias, so two of the six reasons have zero carriers there and the
...
def test_the_six_shapes_cover_the_whole_vocabulary():
