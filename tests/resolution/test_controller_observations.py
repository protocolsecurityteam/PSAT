...
from services.resolution.tracking import clear_classify_cache, observe_controllers
...
def test_observe_controllers(monkeypatch):
    ...
    first = observe_controllers(plan, "https://rpc.example")
    state["owner"] = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
    second = observe_controllers(plan, "https://rpc.example")
...
def test_observe_controllers_decodes_projected_struct_address(monkeypatch):
...
def test_observe_controllers_heartbeats_while_reading_latest_block(monkeypatch):
...
def test_observe_controllers_handles_reverting_getter(monkeypatch):
    plan: ObservationPlan = {
        "schema_version": "0.1",
        "contract_address": "0x1111111111111111111111111111111111111111",
        "contract_name": "Mock",
...
def test_observe_controllers_recovers_immutable_getter_via_impl_fallback(monkeypatch):
    ...
    recovered = observe_controllers(plan, "https://rpc.example", getter_fallback_address=impl)
    ...
    nulled = observe_controllers(plan, "https://rpc.example")
...
def _avs_operator_plan(instance: str) -> ObservationPlan:
    return {
        "schema_version": "0.1",
...
def test_observe_controllers_attributes_beacon_owner_to_instance(monkeypatch):
    ...
    baseline = observe_controllers(plan, "https://rpc.example")
    assert "beacon_owner" not in baseline["controller_values"]

    snapshot = observe_controllers(plan, "https://rpc.example", beacon_address=beacon)
...
def test_observe_controllers_skips_beacon_owner_when_zero_or_reverting(monkeypatch):
    ...
    zero_snap = observe_controllers(plan, "https://rpc.example", beacon_address=beacon)
    assert "beacon_owner" not in zero_snap["controller_values"]

    mode["owner"] = "revert"
    revert_snap = observe_controllers(plan, "https://rpc.example", beacon_address=beacon)
    assert "beacon_owner" not in revert_snap["controller_values"]


def test_observe_controllers_preserves_role_identifier_for_capability_resolver(monkeypatch):
    plan: ObservationPlan = {
        "schema_version": "0.1",
        "contract_address": "0x1111111111111111111111111111111111111111",
        "contract_name": "Mock",
...
# ---------------------------------------------------------------------------
# observe_controllers parity: parallel + sequential produce identical output.
# ---------------------------------------------------------------------------
...
def _snapshot_parity_helper(monkeypatch, fanout: str):
    ...
    return observe_controllers(plan, "https://rpc.example")


def test_observe_controllers_parity_parallel_vs_sequential(monkeypatch):
...
# ---------------------------------------------------------------------------
# Phantom-EOA regression. observation_plan admits non-address state vars (uints,
# bools, mappings) so the *event* pathway can watch them, on the documented
# promise that "the poller filters by type_kind itself". observe_controllers
...
# (recursive.py:911), and permission_index, all of which read ``value`` as
...
def test_observe_controllers_skips_non_address_state_vars(monkeypatch):
    ...
    def _sv(source: str, read_spec: Any) -> ControllerInstruction:
        return {
...
def test_observe_controllers_keeps_address_var_with_missing_type(monkeypatch):
...
def test_observe_controllers_keeps_non_primitive_controllers(monkeypatch):
    ...
    cvs = observe_controllers(plan, "https://rpc.example")["controller_values"]
...
def _multi_controller_plan(target: str) -> ObservationPlan:
    def _sv(source: str) -> ControllerInstruction:
        return {
            "controller_id": f"state_variable:{source}",
            "label": source,
            "source": source,
...
def test_observe_controllers_multicall_prewarm_parity(monkeypatch):
    ...
    off = observe_controllers(plan, "https://rpc.example")

    monkeypatch.setattr("services.resolution.tracking._SNAPSHOT_MULTICALL_ENABLED", True)
    on = observe_controllers(plan, "https://rpc.example")
