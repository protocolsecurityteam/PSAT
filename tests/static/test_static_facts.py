...
def _semantic_function(analysis: StaticFacts, signature: str) -> SemanticFunctionSummary:
...
def _tracked_controller(analysis: StaticFacts, label: str) -> ControllerTrackingTarget:
...
def test_collect_static_inputs_returns_semantic_artifacts(tmp_path):
    """The worker-facing entrypoint
    ``collect_static_inputs`` returns the semantic
...
def test_modifier_helper_auth_structure_recovered(tmp_path):
    ...
    # ``target.ping`` is the body sink. The
    # modifier ``requiresAuth``'s ``auth.canCall`` is now a guard-origin sink,
    # The guard-origin call remains distinct from the body call in sink evidence.
    assert any(sink_id.endswith(":external_call:target.ping") for sink_id in manage["sink_ids"])
    ...
    assert any(sink_id.endswith(":state_write:hook") for sink_id in set_hook["sink_ids"])
