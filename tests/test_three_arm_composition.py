...
# The only way a composed entry publishes ``basis.arm == "gating_authority"``.
...
@pytest.mark.parametrize(
    ...
    [
        (CA.deletability_plane(host=_DELETES_THE_VAULT_AUTHORITY), P.DELETABILITY_ARM_HOST, "setAuthority"),
        (
...
def test_case3b_two_hops_with_a_qualifying_row_republishes_and_names_it(fold, deletability, arm, setter_name):  # noqa: F811
    """POSITIVE ARM, mandatory. Length 2 and the figure survives under either arm
    — a hop-count implementation withholds it, a route-only rule withholds it,
    and only the join licenses it (arms per CAP-A §R1.3)."""
    row = _gate_row(
        CA.composed_document(fold, deletability=deletability, routes=_AUTHORS_THE_AMOUNT_AT_T, case=_two_hop_case())
    )
    ...
    # THIS setter's selector: a basis naming setUserRole beside setAuthority's
    # selector is a licence a reader cannot re-check (B2-R N4).
    ...
    # The licensing row's contract is the one thing the two arms do not share.
