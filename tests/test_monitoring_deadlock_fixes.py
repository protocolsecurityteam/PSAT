...
``db_session`` fixture), stubbing only the wire — ``rpc_batch_request`` and,
...
def test_deadlock_at_stamp_isolates_chunk_and_reports_partial(db_session, monkeypatch):
    ...
    with (
        patch("services.monitoring.unified_watcher.rpc_batch_request", side_effect=_rpc_mock(returns)),
...
def test_deadlock_in_reanalysis_autoflush_is_isolated_not_fatal(db_session, monkeypatch):
    ...
    def _rpc(url, calls):
        ...
        return [_word(ADDR(190)) for _ in calls]

    captured: list[dict] = []
    event.listen(engine, "before_cursor_execute", _abort_update)
    try:
        with (
            patch("services.monitoring.unified_watcher.rpc_batch_request", side_effect=_rpc),
            patch(
...
def test_cleansed_owner_absent_state_emits_no_phantom(db_session, monkeypatch):
    ...
    with patch(
        "services.monitoring.unified_watcher.rpc_batch_request", side_effect=_rpc_mock({ADDR(1): _word(ADDR(200))})
    ):
        events = poll_for_state_changes(db_session, "http://rpc")
