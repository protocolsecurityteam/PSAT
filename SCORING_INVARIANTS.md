...
| `claims[].witness.observed.observed_reach_floor_usd` | **0 on this run** (B12); wired end-to-end in producer and projection | **OPTIONAL-by-witness — three-state, and GATED on `reach_indeterminate`** | the acting deployment's own balance, published as a **lower bound** on the branch where nothing was witnessed leaving a holder. **It is only ever emitted beside `reach_indeterminate: true`** (`recipes.py` writes the pair together), so a consumer that adopts the number without reading that flag has read a **B1/B2 balance-plane figure as a measured reach**. **THREE STATES, amended 2026-07-31 (W3) — the key's own presence is the third.** (1) **present, positive** — a balance row for the acting deployment was read and summed above zero: a witnessed lower bound, and the only adoptable state. (2) **present, `0.0`** — a row WAS read and summed to zero. Still `not_determined`, not a measured zero, because `coalesce(sum(usd_value), 0)` collapses an **all-unpriced** sheet into the same `0.0` (B8.1a): a present zero means "an empty priced sheet **or** an unpriced one". (3) **ABSENT beside `reach_indeterminate: true`** — **this is the failure/absence path**: the INNER join in `selection.py` produced no `deployment_balance` entry at all, so nothing about the acting deployment's balance was witnessed and there is no floor to state. It is **never `null`, never a defaulted `0.0`**. Before W3, `selection.py:1369` defaulted state (3) into state (2) — `.get(acting, _ZERO_USD)` — so the honest absence the join exists to produce reached no consumer; `acting_balance_usd` is now `float \| None` end to end (`Candidate` → `calldata.ValueOutPlanInputs` → `orchestrator` → `recipes.value_out` → `_add_reach`) and `_add_reach` omits the key. **Consumption obligation:** read the **KEY's presence**, never a `.get()` with a numeric default — an absent key resolves to `not_determined` exactly as a `0.0` does, and folding it to zero re-mints the defect; then **gate on `reach_indeterminate == true`** and use the value **as a floor only when strictly positive**; the floor may RAISE a weight, never cap one. Adopting a `0.0` — or a zero conjured out of an absent key — reproduces exactly the "$0 reach on a zero-balance router that can move millions" defect the field was added to fix. |
...
| `claims[].witness.effect_verdict_id` | 69, all distinct | **CONFIDENCE**; obligation = `cite`/join, MANDATORY — the join key for B1/B3 fields that are not projected. It moves no number itself, but a scorer that skips it silently loses every unprojected B2/B3 field, several of which ARE gates. |
| `claims[].tier` | `standard_exact` 353, `idiom_structural` 100, `behavioral_observed` 69, `policy_derived` 19 | **GATE** (gate + cite + three-state) — NOT cite-only, and this row's old `REQUIRED` was wrong in the other direction too. It never multiplies severity and **never raises**, but it **withholds** an escalation needing a behavioural proof (`caller_arbitrary`) and `policy_derived` must **BLOCK** the static conjunction — a licensing role, which is what GATE names. Currently collected into `witness_tiers` and never consumed. |
...
load-bearing — before W3, `recipes.py::_add_reach` published
`graph.deployment_balance.get(acting, _ZERO_USD)` as `observed_reach_floor_usd`,
so a `0` entry manufactured by a failed fetch would have become a published
**$0.00** floor on a function that may move millions. The next block records the
repair; the join's absence-production is unchanged and remains the witness.

**REPAIRED 2026-07-31 (Unit W3). The absence now survives to the consumer, and the
floor key is three-state.** `selection.py` used to construct the candidate with
`float(graph.deployment_balance.get(acting, _ZERO_USD))`, collapsing the *absent* key
the INNER join deliberately produced back to `0.0` one step later; `_add_reach` then
published that `0.0` as `observed_reach_floor_usd` alongside `reach_indeterminate:
true`, so the honest absence the INNER join stated reached no consumer.
`acting_balance_usd` is now `float | None` at every hop —
`selection.Candidate` → `calldata.ValueOutPlanInputs` → `orchestrator` →
`recipes.value_out` → `_add_reach` — with no sentinel at any of them, and
`_add_reach` **omits `observed_reach_floor_usd` entirely** when it is `None`.

**The published shape this changes.** `reach_indeterminate: true` with **no**
`observed_reach_floor_usd` key is a NEW witness shape. The floor key is now:
present-and-positive (a witnessed lower bound), present-and-`0.0` (a balance row WAS
read and summed to zero), or **ABSENT** (no `deployment_balance` entry existed — the
failure/absence path, covering "no current row at all", i.e. never fetched, fetch
failed, or holds nothing, which that plane cannot tell apart). The key is **never
`null`**. The three reach states of the branch as a whole are unchanged and still
discriminated by `reach_determined`; what gained a third state is the floor itself.

**Two of the three causes of a `$0.00` floor remain, so the consumer gate STANDS.**
This repair removes exactly one cause — the no-current-row default. The other two are
still one shape: current rows that are all unpriced (`coalesce(sum(usd_value), 0)`,
the 22 of 60 keys in the next paragraph) and a genuinely empty priced sheet. So
`observed_reach_floor_usd == 0.0` remains **`not_determined` — "no proven bound" — and
may never be adopted as a value or a bound**, unchanged as a normative gate on the
field (see the `observed_reach_floor_usd` row in B1 / `FIELDS.md` §2), enforced in the
prototype at `scoring_prototype/scorer_v3.py` — which, as of this unit's review, also
carries a dedicated ABSENT-key arm in the `flow.out` chain (`inst_val = None` + a
`reach_floor_absent` warning), because without it the chain fell through to a
`.get()`-defaulted magnitude one layer up — exactly the shape this repair removed.
**Non-retroactivity (reviewer-measured):** `db/effect_cache.py::merge_residue` merges
`stored || incoming` whenever the verdict stands, so a pre-W3 `effect_verdicts` row
carrying a defaulted `0.0` floor KEEPS it until a verdict flip or behavior-hash
change — the absence never materialises retroactively. Safe because no consumer
adopts a `0.0` (and 0 of 274 rows on the dev corpus carry the key at all), but a
mixed-vintage corpus is expected and must be read through the gate, not assumed
uniform. **Consumption obligation, extended:** an
ABSENT key resolves to `not_determined` too, and must be read as key-absence — a
`.get()` with a numeric default re-mints precisely the defect this unit removed.
Closing the remaining collapse requires carrying the priced/unpriced three-state
through the SQL aggregate rather than coalescing it, which changes
`build_authority_graph`'s published shape and is NOT done here.

**Tests pinning the shape:** `tests/test_reach_indeterminate_branch.py` (byte-exact
payloads for present-positive / present-zero / absent, plus the `value_holders=()`
early return under all three floor states) and `tests/test_effects_value_reach.py`
(`select_candidates` carries `None` for a contract with no balance row and `0.0` for a
priced-zero row; `value_out` publishes no floor key for the `None` case).
...
| `modules_head` | `…->>'modules_head'` | 19 applicable | **CONFIDENCE**; consumption obligation = `cite`, MANDATORY (it is the raw word every other key in this object was decided from) | the raw 32-byte word at `keccak256(abi.encode(address(0x1), uint256(1)))` / `"not_determined"` | read raised, **or returned anything that is not exactly 64 hex nibbles** ⇒ `"not_determined"`. **Cite, never enumerate from** |
...
| `module_set_basis` | `…->>'module_set_basis'` | 19 applicable | **GATE** (gate + cite) — NOT cite-only: `module_set == []` is a proven empty, and so licenses the k/n protective credit at `probe_block`, **only** on basis `storage_linked_list_terminated`. It conditions a sibling that moves a number, so it stays grade-relevant | `"storage_linked_list_terminated"` / `"not_determined"` | not_determined whenever `module_set` is |
...
**The firing count is witnessed too, by a two-table join — it is 37, with zero
repeats on this corpus.** An earlier draft published it as `not_determined` on the
theory that a repeat firing onto an already-created node leaves no trace. It does
leave one, in the EDGE plane: `_edge_key` (`recursive.py:626-641`) returns the full
tuple **including `from_id`** for every relation outside the merge set
`{safe_owner, timelock_owner, proxy_admin_owner}`, and `role_principal` is not in
that set — so two firings from two different gated contracts leave **two distinct
edges**. And a repeat *within* one contract is impossible to begin with:
`_role_principals_from_effective_permissions` accumulates into
`principals.setdefault(address, …)` (`:755`, `:775`), yielding one entry per address
per processed contract, so a second firing REQUIRES a second from-node and therefore
a second edge. **Measured:** all 37 marked `(contract_id, M)` pairs have exactly one
distinct `role_principal` from-node (max 1, 37 edges total) ⇒ **37 firings, 0
repeats.** Use that join as the witness; do not re-run the walk to answer this.
**Distinct V is unaffected either way** — see the robustness note below — so **16**
is stable regardless.

**Why distinct V is robust — and the corpus fact it currently rests on.** The
probe's V is `gated_contract_address = address` (`recursive.py:1483`), the
**walk-processed** address, which is NOT the same thing as the persisted row's
`contract_id` address that the marker-lens query groups on. On this corpus they
coincide, and that is measured rather than assumed: **all 163 `role_principal` edges
are root-anchored** (`from_node_id` equals the contract's own address, 163/163 over
64 contracts) and **all 37 marked rows sit at `depth = 1`**. So V-by-walk and
V-by-row are the same 16 addresses here. **A depth ≥ 2 projection would break that
identity** and is not covered by this sentence — re-measure before extending the
figure to any corpus where the walk descends past the root.
...
| `backlink_getter` | `…->>'backlink_getter'` | 37 applicable | **CONFIDENCE**; obligation = `cite`, MANDATORY | the literal probed selector, `"vault()"` | never absent while the object exists |
| `gated_contract_address` | `…->>'gated_contract_address'` | 37 applicable | **CONFIDENCE**; obligation = `cite`, MANDATORY — a verdict read without its subject is mis-attributed, but the address itself moves no number and licenses no credit (this object is B14 cite/gate, never calibrate) | the V this verdict is ABOUT — structural, always published, so the row is self-describing | never absent while the object exists |
...
|---|---|---|---|---|
| `not_selected[]` | artifact `selection_summary` | **GATE** (gate + cite) — NOT cite-only | one `{address, chain, rank_score, reason}` per **ranked** candidate that did not become a child. `reason` ∈ `budget_exhausted` \| `chain_not_enabled` \| `existing_job` \| `in_cascade_dedupe` | `[]` means "no ranked candidate was dropped" — proven, because the producer enumerates every non-selected ranked entry, including on the budget-already-full early return |
| `pre_rank_excluded[]` | artifact `selection_summary` | **GATE** (gate + cite) — NOT cite-only: "nothing was dropped" is licensed **iff** `not_selected == [] AND pre_rank_excluded == []`, a conjunctive precondition on a coverage claim | one `{address, chain, reason[, effective_confidence]}` per row removed **before** ranking. `reason` ∈ `below_confidence_threshold` \| `superseded_impl_anchor` | `[]` means "no row was removed upstream of the ranking" |
| `perimeter_spawn_summary` | artifact, policy stage | **GATE** (gate + cite + three-state) — NOT cite-only: `walked == false` ⇒ the three lists are a PREFIX and coverage reads `not_determined` | `{site, budget, budget_used, spawn_depth, queued[], omitted[], out_of_population[], walked}` | `omitted` reasons ∈ `budget_exhausted` \| `depth_exhausted` \| `chain_not_enabled` \| `zero_address` \| `invalid_address`. `walked=false` ⇒ the three lists are a PREFIX, not a partition — read no absence from them |
...
| `probe_address` | `.probe_address` | present whenever the witness exists | **CONF**; obligation = `cite`, MANDATORY | The address the read was issued against — the **runtime** deployment address, never `contracts.address`. No absent state while a witness exists. | **cite.** Without it `slot` + `raw_word` are not replayable. |
...
| `slot` | `.slot` | with the witness | **CONF**; obligation = `cite`, MANDATORY | The descriptor's storage location. Keccak-anchored or layout-derived; absent when the descriptor had none. | **cite.** |
...
| `executor_address` | `.executor_address` | populated iff kind positive | **CONF**; obligation = `cite`, MANDATORY. The gating is done by `executor_kind` and the CHECK-bound `executor_classification_source`/`_classified_type` (all GATE); the emitter address itself moves no number | the marker's emitter · — · NULL | NULL |
...
| `executor_call_targets` | `.executor_call_targets` (JSONB) | gated on `timelock_routed` | **CONF**; obligation = `cite`, MANDATORY. Verified 2026-07-30: its reader `executor_call_targeted_proxy` has **no production consumer** (only `tests/test_upgrade_executor_fold.py`), so it feeds no gate and no arithmetic today | the `target` word of each `CallExecuted` · — · SQL NULL | NULL — and `executor_call_targeted_proxy()` returns `not_determined`, never `false`, for a `safe_direct` tx. **Why the per-target join is not optional, measured from the receipt of `0xc9c80e5b…` (2026-07-30, anchoring `FIELDS.md` §13's "24 of the 26"):** that receipt carries **169 logs**, of which **26 are `Upgraded` from 26 distinct emitters** and **86 are `CallExecuted` naming 26 distinct targets**; **24 of the 26 `Upgraded` emitters are targets** and **2 are not** — `0x3c55986cfee455e2533f4d29006634ecf9b7c03f` and `0xd789870bea40d056a4d26055d0befcc8755da146`, the second of which IS a stored protocol-1 proxy. Joining every log in the transaction to the executor would attribute those two to a `CallExecuted` that never named them. Note the three nested denominators, none of which is the other: **26** `Upgraded` emitters in the receipt ⊃ **21** stored `upgrade_events` rows for the tx ⊃ **19** whose proxy has a protocol-1 `contracts` row |
...
| `as_of_block` | `.as_of_block` | 11 | **CONF**; obligation = `cite`, MANDATORY (inv.10) | present iff `holders` non-NULL | `holders` NULL ⇒ NULL | cite (inv.10 — a mutable now-fact carries its height) |
...
**Wired 2026-07-31 (Unit W1, feat/scorer-gaps); the deferral above is
DISCHARGED.** `ResolutionWorker.process()` now calls
`resolve_role_holder_planes` + `persist_role_holder_planes` immediately after
dependency-edge emission, isolated in its own `try/except` +
`record_degraded("resolution_role_holder_plane")` arm so a role-plane failure
cannot fail the resolution stage. Registry identity is the **runtime address**
(`proxy_address or job.address`), never the implementation. The gate is cursor
**existence** on both `RoleGranted` and `RoleRevoked` topics, chain- and
address-scoped — warmth is not required, so a cold cursor still mints a
withheld row (`holders` NULL / `coverage` `partial`, never `[]`). Every count
in this entry — 11 keys / 3 registries, 10/11 named, 20/20 pinned agreement —
was measured by running the resolver in-process against the local replica and
pinned chain reads; **the table is still empty in every environment** because
no pipeline run has executed the wiring (the replica predates it). The
end-to-end population proof is deferred to the next live run. **The meaning of
absence has therefore CHANGED:** an absent row on a both-cursors registry now
means "the module refused or found nothing to confirm" (`not_determined` — a
registry with both cursors and zero AC logs still correctly yields no row),
no longer "no producer exists". Adversarially reviewed (verdict UPHELD): the
reviewer independently re-measured the pinned reads at 25643300, the
cold-cursor NULL semantics, and the proxy-not-impl registry identity.
...
#### Deferral DISCHARGED 2026-07-31 (Unit W1, feat/scorer-gaps)

The plane now has a scheduled writer: `services/monitoring/restaking_cycle.py`
(`refresh_restaking_plane`: discover emitters → `enroll_restaking_fold` →
`read_positions` → `persist_positions`), run as a **fourth supervised loop** in
`workers/protocol_monitor.py` — a sibling failure domain to the TVL loop
(`tvl.py` untouched; the `Supervisor` isolates each loop in its own thread, and
the cycle additionally catches per protocol). New `PubkeyLinked` cursors enroll
at the **proxy** `0x8b71140a…` (emitter-witnessed, never the impl row 591),
seeded at creation−1 (17174452 — expect the stated one-time ~8.47M-block
window-start drag on that address group), carrying
`enrollment_basis='tracked_topics_asserted'` (the exactness allow-list still
refuses it — honest provenance, not a semantics change). Heartbeat name
`protocol_restaking`; a failed pinned-head read publishes `partial=True`, never
a healthy cycle. **The replica still has zero rows** — it predates the wiring;
the "3 reads/node/cycle" figures remain the spec's expectation until the next
live run, which is the outstanding end-to-end proof. Adversarially reviewed
(UPHELD): the reviewer independently reproduced the pinned EPM discriminators
at 25643300 — including that the strategy near-miss returns byte-identical
shares, so the witnessed-strategy gate is load-bearing — and confirmed the
26-nodes-0-shares result is published unmodified.
...
| `mutability` | **CONF** | `constant` \| `immutable_in_implementation` \| `mutable`; NULL unless `binding=state_variable` | a **DECLARATION CLASS**, explicitly NOT "a writer exists" — that is B20.2's question, answered on its own evidence. `immutable_in_implementation` is spelled the long way because an `immutable` is inlined into the IMPLEMENTATION's bytecode: behind a proxy it is **not** an invariant of the address a consumer prices against. (The earlier "7 of the 10 state-variable rows are proxied" split is **unreproducible from persisted data** — the receiver descriptor postdates every stored `effects` artifact, so neither 7 nor 10 has a run-plane basis. The measurable claim, W2-reviewer-verified at 25643300: **every deployment the asset plane reads carries a nonzero ERC-1967 implementation slot (7/7 deployments), and every one of those jobs carries `request.proxy_address`** — the qualitative conclusion stands strengthened) |
...
#### B20.3 Flow-sink asset address — `artifacts["flow_asset_addresses"]` (W2, 2026-07-31; adversarially reviewed, UPHELD on every field)

Written by the RESOLUTION stage, not the static one: B20.1 publishes the
compiler-minted `auto_getter_selector` but resolves no address, because the
static stage has no proxy knowledge and cannot know which deployment to read.
This plane dereferences that selector at the RUNTIME address — the proxy an
implementation's code executes at, never the implementation its `contracts` row
is keyed to.

Keyed by `(chain_id, deployment_address, asset_getter_selector)`: the envelope
carries the first two, each row the third. **Identity is the minted selector,
never the variable name** — solc refuses to let a hand-written function collide
with an auto-getter, so the declaration→selector binding is 1:1 and enforced by
the compiler, which `variable + "()"` is not (inv.2). `receiver_variables` is
carried for display and source-join only.

**Only a `binding = state_variable` descriptor with `visibility = public` and a
well-formed minted selector is dereferenced.** Well-formedness is gate-checked;
**nullarity is a producer invariant, not gate-enforced** — four bytes cannot be
arity-checked, and the guarantee lives at the descriptor's mint site
(`effects.py` populates `auto_getter_selector` only from a public
state-variable declaration with no parameters; every other construction site
hard-codes `None`, so a hand-written getter structurally cannot mint one). The
corpus's `getTokenOut()` row is exactly the counter-example the gate exists
for: `IERC20 tokenOut = IERC20(getTokenOut())` binds a LOCAL, and at 25643300
`getTokenOut()` answers `0xcd5fe23c…` while `tokenOut()` REVERTS. Pairing a
receiver with a same-named hand-written getter would publish an address for a
function that does not exist; that row mints no row and places no call, and
stays `not_determined`.

**The invariant enum has no closed member.** Behind a proxy an `immutable` is
inlined into the IMPLEMENTATION's bytecode, so it is not an invariant of the
address a consumer prices against — the upgrade authority can re-point the proxy
at an implementation carrying a different constant. `immutable` / `true` /
`closed` are BANNED values on `asset_identity_invariant`.

| field | status | three-state | failure / revert / absence path |
|---|---|---|---|
| `asset_address_status` | **GATE** | `resolved` \| `observed_zero_address` \| `not_determined` | The gate for every other field on this plane. `observed_zero_address` is a COMPLETED read of a real zero — kept distinct from `not_determined` because collapsing them makes "never read" and "read, and it is zero" byte-identical. It is never a priceable address and is never silently dropped |
| `asset_address` | **REQ** | present iff `asset_address_status = resolved`; else **ABSENT** | Absent — not null, not `0x0…0`, not `""`. Reached by revert, transport failure, `"0x"`, a short word, a whitespace-padded word (`bytes.fromhex` ignores ASCII whitespace, so an explicit post-conversion `len == 32` check is load-bearing), an over-long word, a non-canonical word whose high-order 12 bytes are set, or a well-formed zero word. **Never minted** |
| `observed_at_block` | **REQ** | present iff `asset_address_status ≠ not_determined` | **Inseparable from `asset_address` by construction**: both are emitted from one `AssetObservation`, which raises on a missing or non-positive height, so a resolved address without its block is unrepresentable. Emitted only from an explicitly pinned height (`pin_probe_block`); a height that cannot be pinned suppresses the whole plane rather than falling back to `"latest"` |
| `observed_block_hash` | **CONF** | present iff `observed_at_block` is present AND the hash read succeeded | Absent on hash-read failure. The height still stands on its own; only replay-after-reorg is weaker |
| `asset_identity_invariant` | **REQ** | `redirectable_by_upgrade_authority` \| `not_determined`; present iff `asset_address_status = resolved` | **BAN: no closed/true/`immutable` member exists.** `redirectable_by_upgrade_authority` is earned only where the deployment is PROVEN proxied. **Basis: the stage's `request.proxy_address`** — an earned fact about where this code executes. **The upgrade authority itself is NOT enumerated by this plane**: the token says "someone with upgrade authority can redirect this", never who — a consumer must not look here for the authority. "No proxy was proven" is not "no proxy exists", hence `not_determined`, never a closed value |
| `not_determined_reason` | **CONF** | `call_did_not_answer` \| `malformed_return_word`; present iff status is `not_determined` | Diagnostic only, **no consumption obligation**. Revert and transport failure are ONE reason deliberately: a bare `"execution reverted"` arrives with no `error.data` exactly as a provider-side failure does, so the node's text is not a reliable discriminator |
| `declared_mutability` | **CONF** | `constant` \| `immutable_in_implementation` \| `mutable` \| null | Carried verbatim from B20.1 and **never promoted**: it is a DECLARATION CLASS, and an `immutable` declaration still publishes `asset_identity_invariant = redirectable_by_upgrade_authority` behind a proxy. `null` when two folded sites disagreed — one declaration cannot be two classes |
| `asset_getter_selector` | **REQ** | always present | The row key. Refused unless exactly `0x` + 8 hex nibbles — an over-long or truncated string still addresses SOME function, and calling four bytes nobody minted is a fabricated positive. A malformed value mints no row and is logged, never swallowed |
| `sink_ids` | **REQ** | always present, sorted | The join back to `sinks[].id` / `witness.sink_receivers{}`. N sinks sharing one minted selector fold to ONE row and ONE read |
| `receiver_variables` | **CONF** | always present, sorted | **Display and source-join ONLY. `variable + "()"` is the banned shape this plane replaces.** Two receivers sharing a name but not a selector are two assets and do not fold |
| `deployment_proven_proxied` (envelope) | **REQ** | bool | The sole licence for `redirectable_by_upgrade_authority`. False means unproven, NOT proven-unproxied |
| `probe_block` / `probe_block_hash` (envelope) | **REQ** / **CONF** | int / str-or-null | The single height every row in the payload was read at. Absent envelope ⇒ absent artifact ⇒ the plane did not run |

**Absence of the artifact is `not_determined`, never a proven-empty plane.** It is
withheld wholesale when the `effects` artifact is missing, when no licensed
receiver exists, when no runtime address is known, or when no block could be
pinned. **Idempotent**: upsert on `(job_id, name)`, rows sorted by selector, so a
re-run replaces the payload wholesale at a new pinned height.

**Small populations (B14 — zero-instance arms).** On this corpus every published
row is `resolved` (6/6 pairs answered at 25643300, independently reproduced by
the adversarial reviewer): `observed_zero_address`, `not_determined`, and BOTH
`not_determined_reason` values have **ZERO observed instances** — they are
test-only arms. The trap row mints no row at all rather than a not-determined
one. Cite/gate only; never calibrate on them; do not infer they are unreachable.

**Consumption obligation.** `token_identity` may be satisfied ONLY from
`asset_address_status = resolved`, and the consumer MUST carry
`asset_identity_invariant` forward with the address: pricing a
`redirectable_by_upgrade_authority` asset as a fixed identity re-states an
upgrade-authority risk as an asset fact. `observed_zero_address` MUST NOT reach a
pricer and MUST NOT be read as "no asset" — it is one observed word, not a
declaration.

### B21. FP→control-graph materialization — the node an FP row always implied (W5)
...
(`effective_permissions.py:701`), so an address in neither had no
`control_graph_nodes` row — and with no node, every node-driven spawn path
(perimeter at resolution `resolution_worker.py:294`, at policy refresh
`policy_worker.py`) was structurally blind to it.
`reconcile_control_graph_types` — the one FP→CGN fold — is UPDATE-only, so a
principal that never entered the graph could never re-enter it. **Each upstream
refusal is witness-correct** (a `_ROLE_DISSOLVING_TRACE_STEPS` trace leaves
`authority_roles` JSON null on 231 rows; `authority_roles == []` on 127 more
means authority that is not role-keyed); the defect was **"role not_determined"
being consumed as "principal does not exist"**. Measured on the PR-161 replica:
**73 addresses / 411 of 1,200 FP rows (34.3%) / 240 gated functions across 43 of
93 contracts**; 72 of the 73 have no `contracts` row at all, so raising the
discovery `analyze_limit` cannot reach the class.

**The unit of the plane is the ANCHOR, not the address.** CGN is per-contract —
the walk writes one row per `(contract_id, deployment)` scope — so the 73
addresses correspond to **189 `(contract, deployment, address)` pairs** across
**83 anchors** (413 pairs total, 189 of them unminted): `eoa` 74 pairs / 33
addrs, `safe` 50 / 7, `contract` 49 / 38, `timelock` 16 / 1. A consumer counting
addresses from rows must dedupe.

#### B21.1 The minted node — `control_graph_nodes` where `details.control_graph_basis = 'fp_materialization'`

**What the row asserts, and only this: the FP row proves this address is a
resolved principal of a gated function on the anchor contract.** Everything else
is `not_determined`, and the fields below are the pinned form of that refusal.

| field | status | three-state | failure / absence path |
|---|---|---|---|
| `details.control_graph_basis` | **CONF (provenance, no consumption obligation)** | the string `fp_materialization`; key absent on walk-produced nodes | **BANNED as a trust boundary.** It records what produced the row and nothing more. It is NOT what admits the node to the analysis perimeter, and an earlier draft that both used it that way and claimed "nothing else may write it" was wrong on both counts: `details` is free-form JSONB that the walk copies VERBATIM out of upstream principal payloads with no key allowlist, so any producer able to put a key in a principal's details can forge it — demonstrated by a hand-crafted node that passed the gate and spawned a job. Admission is decided by an explicit minted-address set the caller passes to `queue_discovered_contracts`, a construction the data plane cannot reach |
| `analyzed` | **BAN as a completeness witness on these rows** | always `false` | **never stamped `true`.** `true` would make the perimeter spawn on a node no walk ever produced. On these rows `false` means *never offered to a walk* — the DEFINITION of the population, not a verdict about the address, and not "this stage examined it and declined" |
| `analysis_state` | **GATE** | NULL (`not_determined`) at mint | the walk never ran on this node, so no state is derivable. A later `reconcile_control_graph_types` may fill it from the row's own fields — `not_analyzable` for `safe`/`eoa` (true: outside `ANALYZABLE_TYPES`), and **NULL stays NULL for `timelock`/`proxy_admin`/`contract`**, which is correct: an analyzable contract inside the horizon with no recorded failure has no determined state |
| `graph_max_depth` | **GATE** | always NULL on these rows | the walk's horizon is a completeness claim ABOUT A WALK. No walk covered this node, so stamping one would publish a horizon that never applied. `depth` alone therefore cannot distinguish "cut off" from "not attempted" here — read the basis key first |
| `depth` | **CONF (cite)** | anchor contract's node depth + 1; **NULL when the anchor's own node is absent** | not guessed at 0 or 1. NULL = the anchor's depth was not witnessed |
| `resolved_type` | **REQ** | verbatim from `function_principals.resolved_type` | **the mint REFUSES on both undetermined and conflicting input.** NULL type ⇒ `resolved_type_not_determined`; two types for one principal at one anchor ⇒ `resolved_type_conflict` and no row. 0 of the corpus's 413 (anchor, address) pairs reach the conflict arm, but the two candidate types straddle the job / no-job split, so a first occurrence must surface as a refusal, never a picked winner |
| `node_type` | **GATE** | `contract` iff `resolved_type ∈ ANALYZABLE_TYPES` (`contract`/`timelock`/`proxy_admin`), else `principal` | **this is the whole job / no-job split, and it is the EXISTING walker gate — W5 adds no job-creation logic.** `safe`/`eoa` mint `principal`, which `queue_discovered_contracts` rejects as `not_contract_node`: node yes, job never |
| `label` | **BAN** | **always NULL** | **not merely unused — actively harmful if set.** `queue_discovered_contracts` derives the child's `Job.name` / `request["name"]` from the node, and `company_overview` reads the same field for display. A constant here (an earlier draft wrote `"capability principal"`) publishes a description of the EDGE as the IDENTITY of the CONTRACT, on every spawned child, and REGRESSES what these principals already displayed via the FP type fold (`timelock` / `safe`). The job-name fallback no longer consults `label` at all — `contract_name or address`. `resolved_type` carries the only proven noun |
| `contract_name` | — | always NULL | not witnessed by an FP row |
| `details.fp_function_count` | **CONF (cite)** | count of FP rows at this anchor naming this address | a floor on the gated surface, not a claim about the whole contract |
| `details.fp_origins` / `details.fp_principal_types` | **CONF (cite)** | sorted distinct non-NULL values across the contributing FP rows; `[]` = no non-NULL value on any row | descriptive provenance. `[]` and absent are the same here: the lists are always emitted |
| intrinsic config (`owners`, `threshold`, `delay`) | — | **never minted here** | key absent at mint. `reconcile_control_graph_types` folds it in from the FP row's own `details` on its next run — where the witness already lives; duplicating it would create a second writer for one fact |

**Durability — a consumer must not treat these rows as stable, and must not
treat a budget cut as recoverable.** `replace_control_graph_rows` deletes
wholesale within the same `(contract_id, deployment)` scope, so a minted row
cannot be made durable against it. The invariant is RE-MINT BY ORDERING: the
mint runs strictly after the last rewrite in the job's stage sequence, and is
idempotent, so a row absent between a rewrite and the following mint is a
**transient** — absence is `not_determined`, never "this address is not a
principal". **But the same rewrite is what makes a budget cut permanent** — see
B21.3.

#### B21.2 The edge — `control_graph_edges.relation = 'capability_principal'`

| field | status | three-state | failure / absence path |
|---|---|---|---|
| `relation = 'capability_principal'` | **REQ**, member of `CONTROL_EDGE_RELATIONS` | present / absent | **Deliberately NOT `role_principal`.** That relation asserts a WITNESSED ROLE, and the population reaching this pass is precisely the one `capability_role_grants` REFUSED to assert a role for. Minting `role_principal` would publish the exact claim the upstream declined. This one asserts only "resolved principal of a gated function on the from-node" |
| direction | **GATE** | `from_node_id` = `address:<anchor contract>`, `to_node_id` = `address:<principal>` | same stored direction as every other control edge — the row stores *contract controlled BY controller*, so the authority direction is the REVERSE of the stored edge (`selection.py:288`) |
| `label` | — | always NULL | no label is witnessed. Never read one off this relation |
| `notes` | **CONF (cite)** | `["functions=<n>"]` | the same floor as `details.fp_function_count` |
| `source_controller_id` | — | always NULL | there is no controller-value row behind this edge |

**It moves NO NEW value through the effects closure.**
`services/effects/selection.py:349-355` (`build_authority_graph`) already folds
`function_principals` in directly as `principal → contract`, so this edge
duplicates an authority link the closure carried anyway; what is new is that the
link is reachable in the **table plane** that Surface, chat and enrollment read
instead of the FP rows. A scorer must not double-count it against the FP-derived
arm.

#### B21.3 The ledger — artifact `fp_materialization_summary` (site `fp_materialization`)

Same `PerimeterSpawnResult` machinery as `perimeter_spawn_summary`, **two
planes**, and reading them as one is an error:

* the three inherited dispositions **partition** every FP principal considered,
  on the question *will this address be offered to the walker as an analysis
  candidate?* — `queued` (offered) / `omitted` (could have been, was not:
  `budget_exhausted`, `chain_not_enabled`) / `out_of_population` (never
  offerable: `not_analyzable_type`, `existing_node`, `zero_address`,
  `invalid_address`, `no_contract_anchor`, `anchor_contract`,
  `resolved_type_not_determined`, `resolved_type_conflict`) — **a partition only
  when `walked == true`**;
* `minted[]` is the **ORTHOGONAL** plane — the nodes actually INSERTed and
  COMMITTED, exactly `queued` ∪ the `not_analyzable_type` entries. Minting a node
  and offering a job are different acts, and the whole point of the unit is that
  a Safe gets the first without the second.

| field | status | three-state | failure / absence path |
|---|---|---|---|
| `walked` | **GATE** | `true` \| `false` | set at LOOP EXIT and nowhere else, exactly as in `perimeter_spawn_summary`. `true` licenses the partition claim; on `false` the lists are a PREFIX and **their emptiness proves nothing** — read coverage as `not_determined` |
| `minted[]` / `budget_used` | **REQ** | committed rows only | the mint **commits before it records**. The ledger is persisted from a `finally`, on a FRESH session when the primary one is poisoned, so recording first let a rollback publish entries naming rows that do not exist — a positive fact about a row nothing wrote. `queue_discovered_contracts` commits after each `create_job` for the same reason, and the equivalence this ledger claims required the same boundary |
| `out_of_population[].reason = 'not_analyzable_type'` | **GATE** | present per minted `safe`/`eoa` | the mint-side half of the no-job proof. The walker's `not_contract_node` is the other half. **A silent skip on either side is the defect this unit closes** |
| `omitted[].reason = 'budget_exhausted'` | **GATE** | present per cut candidate | **A PERMANENT LOSS ON THIS ANCHOR, NOT A QUEUE.** An earlier draft of this register claimed the tail drains, on the reasoning that `existing_node` precedes the budget gate. **RETRACTED, and the production sequence is what refutes it:** every job rewrites this `(contract, deployment)` scope wholesale (resolution stage, then policy) BEFORE the mint runs, so no minted row survives for `existing_node` to find, and `sorted(candidates)` re-mints the same lexicographic prefix and drops the same tail forever. Measured at a cap of 16 (adversarial-reviewer-reconciled, round 2): **1 of 83 anchors over budget, 11 addresses permanently invisible, 2 contract-typed job candidates lost (`0xc4922d64…`, `0xfd78ee91…`), residual jobs 37 → 35**; the boundary slot mints — the gate bites only after the Nth insert |
| `budget` / `budget_used` | **CONF (cite)** | `PSAT_FP_MATERIALIZE_LIMIT`, default **64** | a **NAMED MODEL CHOICE, never calibrated** (B14) — and, because a cut is permanent, sized to leave **no live tail** rather than to tune: 64 is 2× the observed per-anchor maximum of 31 distinct principals, and **0 of 83 corpus anchors exceed it** (1 exceeds 16). It is a BACKSTOP against a pathological anchor and bounds ROW growth only; fan-out is bounded by `PERIMETER_SPAWN_LIMIT` (8) and `PERIMETER_SPAWN_DEPTH_CAP` (2), both unchanged, and minting costs no RPC. Lowering it re-introduces a permanent tail |
| artifact absence | **GATE** | absent = "this job predates the ledger", **never** "minted nothing" | written from a `finally` and retried on a fresh session; a lost write raises `record_degraded(phase='fp_materialization_summary')` rather than letting the absence lie |

**Small populations (B14).** The `resolved_type_conflict` arm is **0 of 413
(anchor, address) pairs** — live, exercised code, zero observed population; cite
the arm, never calibrate on it. `no_contract_anchor` is likewise 0 (FK-protected
in practice; reachable via a blank `Contract.address`). The `timelock` bucket is
**1 address / 16 anchors** and the `safe` bucket **7 addresses** — cite, never
calibrate.

**Carried forward, NOT closed by this unit:** the `contract`-typed members are
**38 addresses / 49 anchors**, and the investigation flagged obvious third-party
infrastructure among them (`0x2222222d…`, `0x0000c1c0…`). They are in
`ANALYZABLE_TYPES`, so at the shipped cap **37 job-candidate addresses have no
existing job and will spawn one** over successive policy runs (≤8 per job, ≤2
generations). Nothing in this plane distinguishes protocol infrastructure from
third-party, and job-eligibility deliberately stays with the walker's gates —
which do not ask that question. Publishing a protocol-membership verdict here
would be exactly the unwitnessed claim the unit exists to remove.

**Measurement reconciliation (2026-07-31, review round 2).** The round-1 review
and the implementer's replay disagreed on the cap-16 counterfactual (11 vs 10
addresses; residual 34 vs 35). The reviewer's round-2 SQL replay of the code's
exact gate order reconciled it: **11 addresses cut, 2 contract-typed lost,
residual 35** — each earlier figure carried one off-by-one (a boundary-slot
miscount and a rank miscount respectively). At the shipped cap of 64 nothing is
cut and both derivations agree on 37.
